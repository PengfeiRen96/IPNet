...
# PointMLP(joint_num=joint_num, points=1024, embed_dim=64, groups=1, res_expansion=1.0,
...
class PointMLP_refine(nn.Module):
    def __init__(self, joint_num=50, points=1024, embed_dim=64, groups=1, res_expansion=1.0,
                 activation="relu", bias=True, use_xyz=True, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
                 k_neighbors=[16, 16, 16, 16], reducers=[4, 4, 4, 4],
                 de_dims=[512, 256, 128, 128], de_blocks=[4, 4, 4, 4],
                 gmp_dim=64, **kwargs):
        super(PointMLP_refine, self).__init__()
        self.stages = len(pre_blocks)
        self.joint_num = joint_num
        self.points = points
        assert len(pre_blocks) == len(k_neighbors) == len(reducers) == len(pos_blocks) == len(dim_expansion), \
    ...
    def forward(self, xyz, x):
        ...
        for layer in self.finals:
            temp = layer(point_feature)
            point_result = torch.cat((point_result, temp), dim=1)
        return point_result.permute(0,2,1)


def pointMLP(joint_num, **kwargs) -> PointMLP:
    return PointMLP(joint_num=joint_num, points=1024, embed_dim=64, groups=1, res_expansion=1.0,
                 activation="relu", bias=True, use_xyz=True, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
                 k_neighbors=[16, 16, 16, 16], reducers=[4, 4, 4, 4],
                 de_dims=[512, 256, 128, 128], de_blocks=[4, 4, 4, 4],
...
if __name__ == '__main__':
    data = torch.rand(32, 3, 1024).cuda()
    norm = torch.rand(32, 1024, 3).cuda()
    print("===> testing modelD ...")
    model = pointMLP(joint_num=21).cuda()
    out = model(data)
    import time
    a = time.time()
    for i in range(100):
